 ...
 * represents the root's value and a pair of parenthesis contains 
...
public class ConstructBinaryTreeFromString {
	...
	private class TreeNode<T> {
		...
		void printTree(TreeNode<T> head) {
			if( head != null) { 
				printTree(head.left);
	...
	public TreeNode<Integer> str2tree(String s) {
		if (s == null || s.length() == 0) 
			return null;
		int firstParen = s.indexOf("(");
		int val = firstParen == -1 ? Integer.parseInt(s) : Integer.parseInt(s.substring(0, firstParen));
		TreeNode<Integer> cur = new TreeNode<Integer>(val);
	...
	public static void main(String[] args) {
		...
		TreeNode<Integer> result = obj.str2tree("4(2(3)(1))(6(5))");
