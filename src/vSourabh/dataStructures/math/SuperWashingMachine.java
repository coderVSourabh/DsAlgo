 ...
 * washing machines, and pass one dress of each washing
 * machine to one of its adjacent washing machines at the
 * same time .
 * 
 * Given an integer array representing the number of dresses 
 * in each washing machine from left to right on the line, 
 * you should find the minimum number of moves to make all
 * the washing machines have the same number of dresses. 
 ...
 * Explanation: 
 * It's impossible to make all the three washing machines
...
public class SuperWashingMachine {
	...
	public static void main(String[] args) {
		int result;
		SuperWashingMachine obj = new SuperWashingMachine();
		result = obj.findMinMoves(new int[] {0,3,0});
		System.out.println("The number of moves are " + result);

		result = obj.findMinMoves(new int[] {1,0,5});
		System.out.println("The number of moves are " + result);
	}
